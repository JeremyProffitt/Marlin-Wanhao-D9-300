    ...
    while (wait_for_heatup && heaters_heating) {
      ...
      HOTEND_LOOP() {
        if (thermalManager.degTargetHotend(e) && abs(thermalManager.degHotend(e) - thermalManager.degTargetHotend(e)) > TEMP_HYSTERESIS) {
		  ...
		  #elif defined(DWIN_SCREEN_FYS_V1)
...
static bool FunV00B(uint32_t& printedTime)
{
	...
	#endif
	planner.set_position_mm(0,0,current_position[Z_AXIS],0);
#else
	planner.set_e_position_mm(0);
#endif
	planner.buffer_line(0, 0, current_position[Z_AXIS], 4.0, homing_feedrate(Z_AXIS), active_extruder);
	stepper.synchronize();
	planner.buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], 4.0, 30.0, active_extruder);
	stepper.synchronize();
	sync_plan_position_e();
	GLOBAL_var_V001 |= ((uint16_t)0x0001 << MACRO_var_V006);
	clear_command_queue();
...
{
	Ydwin::dwinSendCmdFromString(parser.string_arg);
}
void FunV010()
{
