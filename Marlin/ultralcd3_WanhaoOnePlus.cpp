...
void kill_screen(const char* lcd_msg) {
  ...
  void lcd_main_menu()
  {
	  ...
	  if (!(card.cardOK&&card.isFileOpen()))
	  {
		...
		MENU_ITEM(gcode, "Auto Level Bed", PSTR("G28\nG29\n"));
		#endif
	...
	#endif 
	#ifdef DEVELOPER_MODE
	  MENU_ITEM(submenu, "CONTRAST", lcd_set_contrast);
	#ifdef  POW_BREAK_CHECK_PIN
	  MENU_ITEM(gcode, "Power break", PSTR("M1100"));
	  MENU_ITEM(gcode, "Continue print", PSTR("M1101"));
	#endif
	#endif
	END_MENU();
  }
  ...
  void lcd_cooldown() {
	...
	void myMoveFilament()
	{
		...
		if (filamentStatus & 0x02 && !planner.blocks_queued())
		{
			lcd_preheat_material1_hotend0();
			filamentStatus &= 0xFD;
		}
...
void lcd_update() {
  ...
  if (ELAPSED(ms, next_lcd_update_ms)) {
    ...
    #if ENABLED(ULTIPANEL)
	if (currentScreen == lcd_status_screen || currentScreen == myMoveFilament || defer_return_to_status)
        return_to_status_ms = ms + LCD_TIMEOUT_TO_STATUS;
